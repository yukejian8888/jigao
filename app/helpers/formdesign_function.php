<?php
if (!function_exists('parse_form')) {
    /*
        PHP 解析表单
        template 表单设计器里的Html内容
        fields 字段总数
    */
    function parse_form($template, $fields = 0)
    {
        //获取标签
        ...
        if ($temparr) {
            foreach ($temparr[0] as $pno => $plugin) {
                ...
                foreach ($parse_attr[1] as $k => $attr) {
                    ...
                    if ($attr) {
                        ...
                        if ($tag == 'select' && $attr == 'value') {
                            $attr_arr_all['value'] .= $select_dot . $val;
                            $select_dot = ',';
                        } else {
                ...
                if ($tag == 'checkboxs') /*复选组  多个字段 */ {
                    ...
                    //parse
                    $template = str_replace_once($plugin, $attr_arr_all['content'], $template);
                    $template_parse = str_replace_once($plugin, '{' . $name . '}', $template_parse);
...
if (!function_exists('unparse_form')) {
    /*
    * 编译字段
    * $controller = array(
    ...
    * );
    */
    function unparse_form($form, $form_data = array(), $controller = array())
    {
        $tpl_data = json_decode($form['content_design_data'],true);
        $tpl = $form['content_design_parse'];
        //表格样式
        ...
        if($tpl_data){
            foreach ($tpl_data as $key => $value) {
                ...
                if($value['leipiplugins']=='text'){
                    $string = '<input type="text" value="' . $value['value'] . '"  name="' . $value['name'] . '"  style="' . $value['style'] . '"/>';
                    $temp_html = $string;
                }elseif ($value['leipiplugins']=='textarea'){
                    $string = '<textarea  name="' . $value['name'] . '" id="' . $value['name'] . '" value="' . $value['value'] . '" style="' . $value['style'] . '">';
                    $string .= $value['value'];
                    $string .= '</textarea>';
                    if ($value['orgrich']) {
                        $string .= '<script>
                    ...
                    $temp_html = $string;
                }elseif ($value['leipiplugins']=='radios'){
                    $string = '';
                    foreach ($value['options'] as $item) {
                        ...
                        $string .= '<input type="radio" name="' . $value['name'] . '" value="' . $item['value'] . '" ' . $checked . '>' . $item['value'] . '&nbsp;';
                        $temp_html = $string;
                    }
                }elseif ($value['leipiplugins']=='checkboxs'){
                    $string = '';
                    foreach ($value['options'] as $item) {
                        $checked = '';
                        ...
                        $string .= '<input type="checkbox" name="' . $item['name'] . '" value="' . $item['value'] . '" ' . $checked . '>' . $item['value'] . '&nbsp;';

                    }
                    $value['name'] = $value['parse_name'];
                    $temp_html = $string;
                }elseif ($value['leipiplugins']=='select'){
                    $string = '';
                    //设置过默认值  清空它
                    if (isset($form_data[$value['name']])) {
                    ...
                    $temp_html = $string;
                }elseif ($value['leipiplugins']=='macros'){
                    $temp_html = macros_parse($value);
                }elseif ($value['leipiplugins']=='progressbar'){
                    $string = '';
                    if ($controller['action'] == 'edit') {
                        $value['value'] = floatval($value['value']);
                        $value['value'] = $value['value'] > 0 ? $value['value'] : floatval($value['orgvalue']);
                        $string = '进度 <input type="text" style="width:40px" name="' . $value['name'] . '" value="' . $value['value'] . '"/> %';
                    } else if ($controller['action'] == 'view') {
                        $string = '<div class="progress progress-striped"><div class="bar ' . $value['orgsigntype'] . '" style="width: ' . $value['value'] . '%;"></div></div>';
                    } else if ($controller['action'] == 'preview') {
                        $string = '<div class="progress progress-striped"><div class="bar ' . $value['orgsigntype'] . '" style="width: ' . $value['orgvalue'] . '%;"></div></div>';
                    }
                    $temp_html = $string;
                }elseif ($value['leipiplugins']=='qrcode'){
                    $string = '';
                    //未处理生成二维码
                    $orgtype = '';
                    ...
                    if ($value['value'])
                        $qrcode_value = json_decode($value['value'],true);
                    //print_R($qrcode_value);exit;  //array(value,qrcode_url)
                    if ($controller['action'] == 'edit') {
                        $string = $orgtype . '二维码 <input type="text" name="' . $value['name'] . '" value="' . $qrcode_value['value'] . '"/>';
                    } else if ($controller['action'] == 'view') {
                        ...
                        $string = '<img src="' . $qrcode_value['qrcode_url'] . '" title="' . $qrcode_value['value'] . '" style="' . $style . '"/>';
                        ...
                        $string = '<img src="' . $value['src'] . '" title="' . $value['orgtype'] . '" style="' . $style . '"/>';
                    }
                    $temp_html = $string;
                }elseif ($value['leipiplugins']=='listctrl'){
                    $string = '';
                    //编辑数据
                    $def_value[$key] = !empty($value['value']) ? json_decode($value['value'],true) : '';
...
//                    if ($controller['action'] == 'view') {
                        ...
                        $string .= '<table id="' . $value['name'] . '_table" cellspacing="0" class="table table-bordered table-condensed" style="' . $value['style'] . '">';
                        $string .= '<thead>
...
//                        {
//                            $string .= '<tbody><tr></tr>' . $tb_tf_tr . '</tbody>';
//
//                        } else//刚发起的公文，没编辑值时，默认一行
//                        {
//                            $string .= '
...
if (!function_exists('unparse_data')) {
    //抽出表单提交后 控件的值
    function unparse_data($form, $post_data, $controller = array())
    {
...
if (!function_exists('macros_parse')) {
    function macros_parse($data, $def_value = '', $controller = array())
    {
        $tpl = $data['content'];
        $date_format = '';
        switch ($data['orgtype']) {
