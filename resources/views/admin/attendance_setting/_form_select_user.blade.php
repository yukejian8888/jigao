...
<script>
    ...
    $(function () {
        //添加审批人员
        $('.dogo-js-add-approval').click(function () {
            $('#modal_approval').modal('show');
            var url = '{{route("admin.attendance.get_user_list")}}'+'?time='+Math.random();
            //获取已选择的用户id
            var user_id = new Array();
            $('input[name="need_attendance_people[]"]').each(function () {
        ...
        $('.dogo-js-select-submit').click(function () {
            ...
            $('input[name="checkbox_user"]:checked').each(function () {
                ...
                //增加会员名称，异步获取用户信息
                var url = '{{route("admin.attendance.get_user_info")}}';
                $.ajax({
                    url:url,
                    type:'post',
                    data:{
                    ...
                    success:function (response) {
                        ...
                        if(response.status=='s'){
                            var html_user = '<span class="dogo-warp-approval dogo-js-del-user-id-'+response.data.id+'">'+
                                '<input type="hidden" name="need_attendance_people[]" value="'+checked_val+'"/>'+
                                ...
                                '</span>';
                            $('.dogo-attendance-people').append(html_user);
                        }
                    }
                });
            });
        ...
        //删除会员id
        $('.dogo-attendance-people').on('click', '.dogo-js-del-approval', function () {
            var user_id = $(this).attr('data-user-id');
            layer.confirm('确定要删除吗?', {icon: 3, title:'提示'}, function(index){
                layer.closeAll();
                $('.dogo-attendance-people .dogo-js-del-user-id-' + user_id).remove();
            },function (index) {
                layer.closeAll();
            });
        });
