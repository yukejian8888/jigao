...
@section('content')
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <div class="row">
                <div class="col-md-6">
                    <a href="{{route('item.create')}}" class="btn btn-labeled btn-info"><span
                                class="btn-label"><i class="fa fa-plus"></i></span>添加</a>
                </div>
                <div class="col-md-6">
                    {!! Form::open(['route' => ['item.index'],'method' => 'get','class'=>'form-inline']) !!}
                ...
                <table class="table table-hover table-bordered">
                    ...
                    @foreach($infos as $item)
                        <tr class="dogo-remove-{{$item->id}}">
                            ...
                            <td>
                                <a href="{{route('item.edit',['id'=>$item['id']])}}" title="编辑"><i
                                            class="fa fa-edit"> </i> 编辑</a>&nbsp;
                                <a href="javascript:void(0)" class="dogo-click-delete" title="删除"
                                   data-url="{{route('item.destroy',['id'=>$item['id']])}}"
                                   data-title="{{$item->name}}" data-id="{{$item->id}}"><i class="fa fa-trash-o"> </i> 删除</a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
