...
@section('content')
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <div class="row">
                <div class="col-md-6">
                    <a href="{{route('articlesort.create')}}" class="btn btn-labeled btn-info"><span
                                class="btn-label"><i class="fa fa-plus"></i></span>添加</a>
                </div>
                <div class="col-md-6">
                    {!! Form::open(['route' => ['articlesort.index'],'method' => 'get','class'=>'form-inline']) !!}
                ...
                <table class="table table-hover table-bordered">
                    ...
                    @foreach($infos as $item)
                        <tr class="dogo-remove-{{$item->id}}">
                            <td>{{$item->id}}</td>
                            {{--<td>{{$item->name}}</td>--}}
                            <?php $px = $item['lve']==0?30:$item['lve']*40;?>
                            <td style="padding-left: {{$px}}px;">
                                <?php
                                if( $item['lve'] <> 0){
                                    echo "<img src='../style/admin/images/organization.png'>";
                                }
                                ?>
                                {{$item->name}}
                            </td>
                            ...
                            <td>
                                <a href="{{route('articlesort.edit',['id'=>$item['id']])}}" title="编辑"><i
                                            class="fa fa-edit"> </i> 编辑</a>&nbsp;
                                <a href="javascript:void(0)" class="dogo-click-delete" title="删除"
                                   data-url="{{route('articlesort.destroy',['id'=>$item['id']])}}"
